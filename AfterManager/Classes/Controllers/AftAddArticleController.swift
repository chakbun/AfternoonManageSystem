...
class AftAddArticleController: ZSSRichTextEditor {
    ...
    override func viewDidLoad() {
        ...
        let postArticleItem: UIBarButtonItem = UIBarButtonItem.init(barButtonSystemItem: UIBarButtonSystemItem.done, target: self, action:#selector(AftAddArticleController.postArticleAction(_:)))
    ...
    //MARK: Button Action
    func postArticleAction(_ sender: AnyObject) -> Void {
        let title = "测试"
        ...
        AftBmobManager.sharedInstance.postArticleWithInfo(info: ["title":title, "content":content, "author":author, "authorInfo":authorInfo, "refer":refer]) { (error, isSuccess) -> Void in
